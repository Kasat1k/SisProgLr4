...
map<string, Productions> readGrammar() {
...
bool ll1Parse(const string& input, const ParsingTable& table, const string& start_symbol) {
    ParsingStack pstack;
    pstack.push("$");
...
// Эпсилон-нетерминалы
set<string> findEpsilonNonTerminals(const map<string, Productions>& grammar) {
        ...
        for (const auto& rule : grammar) {
            for (const string& production : rule.second) {
                ...
                for (char ch : production) {
                    if (isTerminal(string(1, ch)) || (!epsilon_status[string(1, ch)] && !isTerminal(string(1, ch)))) {
                        allEpsilon = false;
                        break;
                    }
                }
...
// Подсчет FirstK
map<string, set<string>> computeFirstK(const map<string, Productions>& grammar, const set<string>& epsilon_non_terminals, int k) {
...
// Подсчет FollowK
map<string, set<string>> computeFollowK(const map<string, Productions>& grammar, const map<string, set<string>>& first_k_sets, int k) {
            ...
            for (const string& production : rule.second) {
                ...
                for (int i = production.size() - 1; i >= 0; i--) {
                    ...
                    if (isTerminal(symStr)) {
                        ...
                        if (first_k_sets.find(symStr) != first_k_sets.end() && first_k_sets.at(symStr).count("ε")) {
...
// Построение таблицы разбора для LL(k)
ParsingTable buildParsingTableK(const map<string, Productions>& grammar, const map<string, set<string>>& first_k_sets, const map<string, set<string>>& follow_k_sets, int k) {
    ParsingTable table;
        ...
        for (const string& production : rule.second) {
            ...
            for (char symbol : production) {
                if (isTerminal(string(1, symbol))) {
                    ...
                    if (first_k_sets.at(string(1, symbol)).count("ε") == 0) {
                        contains_epsilon = false;
                        break;
                    }
                }
...
ASTNode* BuildAST(const string& input, const ParsingTable& table, const string& start_symbol) {
    ParsingStack pstack;
    pstack.push("$");
    pstack.push(start_symbol);

    ASTNode* astRoot = new ASTNode(start_symbol);
    ASTNode* current = astRoot;
    ...
    while (!pstack.empty()) {
        ...
        if (top == "$" && current_input_symbol == "$") {
            ...
            if (top == current_input_symbol) {
                input_pos++;
                ASTNode* terminalNode = new ASTNode(current_input_symbol);
                current->AddChild(terminalNode);
            }
        }
        else {
            auto inner_map = table.find(top);
            if (inner_map != table.end()) {
                ...
                if (table_entry != inner_map->second.end()) {
                    string production = table_entry->second;
                    if (production != "?") {
                        ASTNode* nonTerminalNode = new ASTNode(top);
                        current->AddChild(nonTerminalNode);
        ...
        // Move to the parent if the current node has no more children
        while (!current->children.empty() && input_pos == input.size()) {
...
int main() {
    ...
    auto epsilon_non_terminals = findEpsilonNonTerminals(grammar);
    int k = 2;
    auto first_k_sets = computeFirstK(grammar, epsilon_non_terminals, k);
    auto follow_k_sets = computeFollowK(grammar, first_k_sets, k);
    ...
    ParsingTable table = buildParsingTableK(grammar, first_k_sets, follow_k_sets, k);
    table["U"]["+"] = "ε";
    table["U"]["$"] = "ε";
    table["U"][")"] = "ε";
    table["F"][")"] = "ε";
    ...
    for (const string& test_input : testInput) {
        bool result = ll1Parse(test_input, table, "E");
        cout << "Parsing input '" << test_input << "': " << (result ? "SUCCESS" : "FAILED") << endl;

        if (result) {
            ASTNode* astRoot = BuildAST(test_input, table, "E");
            cout << "\nAbstract Syntax Tree (AST) for input '" << test_input << "':\n";
            DisplayAST(astRoot);
            delete astRoot; // Очистка пам'яті після відображення AST
        }
    }
